...
> 运行或管理每日论文推荐 feed。裸 `/daily-eprint` 表示“现在跑一次今天的推荐”；GitHub Actions 只是同一条 pipeline 的无人值守调度器。
...
- `/daily-eprint`：现在跑一次推荐。如果缺少 `config/daily-eprint.yml`，从 wiki 推断默认值后继续。
- `/daily-eprint setup`：从 `config/daily-eprint.yml.example` 创建或修复配置，检查 `.github/workflows/daily-eprint.yml`，并说明需要的 secrets。
- `/daily-eprint status`：检查 config、workflow、schedule、mode、API/e-mail secrets 可用性，以及最近 artifacts。
- `/daily-eprint disable`：把 config 中的 `schedule.enabled` 设为 `false`，或告诉用户需要怎样修改；手动 `/daily-eprint` 仍可使用。
...
2. 读取 `.daily-eprint/run/recommendation-context.json`。基于 arXiv、wiki、Semantic Scholar、DeepXiv evidence，用 LLM 判断推荐质量。写出 `.daily-eprint/run/llm-decisions.json`，字段包括 `decision`、`confidence`、`score`、`rationale`、`wiki_connections`、`signals_used`。在 CI 的 inform mode 中，可用 OpenAI-compatible review LLM 执行：
...
inform 运行只写 `.daily-eprint/` 下的 scratch 文件。`auto-ingest` 中所有持久 wiki/raw 变更都必须来自 `/ingest`。

## Relationships

- `/discover` 回答用户主动提出的 next-read 请求，可来自 anchors、topic 或 wiki 状态；它永不 ingest。
- `/daily-eprint` 监听 fresh arXiv stream，可手动或每日通知。
- `/ingest` 是唯一论文纳入路径。`/daily-eprint` 只能在显式 `auto-ingest` mode 下调用它。
