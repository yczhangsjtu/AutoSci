#!/usr/bin/env python3
"""Deterministic helpers for the /daily-eprint recommendation pipeline.

The slash skill is the policy and LLM judgment layer. This helper keeps the
repeatable parts in one place: config resolution, ePrint feed handling, wiki
...
ARXIV_URL_RE = re.compile(
    r"https?://eprint\.org/(?:abs|pdf)/([A-Za-z0-9./-]+)", re.IGNORECASE
)
...
def _parse_simple_yaml(text: str) -> dict[str, Any]:
    """Parse the small YAML subset used by config/daily-eprint.yml.
...
def load_config(path: Path | None, overrides: dict[str, Any] | None = None) -> tuple[dict[str, Any], list[str]]:
    ...
    if not config_found:
        env_categories = _split_categories(os.environ.get("EPRINT_CATEGORIES"))
        if env_categories:
            cfg["categories"] = env_categories
            notes.append("Using EPRINT_CATEGORIES from the environment.")
...
def _normalize_arxiv_id(value: str) -> str:
    ...
    text = re.sub(r"^eprint:\s*", "", text, flags=re.IGNORECASE)
...
def _fetch_feed(cfg: dict[str, Any], out_path: Path | None = None) -> list[dict[str, Any]]:
    if fetch_arxiv is None:
        raise RuntimeError("tools/fetch_eprint.py could not be imported")
...
def _paper_url(paper: dict[str, Any], eprint_id: str) -> str:
    url = str(paper.get("eprint_url") or paper.get("url") or "").strip()
    if url:
        return url
    return f"https://eprint.iacr.org/{eprint_id}" if eprint_id else ""
        ...
        cards.append(
            {
                ...
                "arxiv_id": _normalize_arxiv_id(_frontmatter_value(text, "eprint") or _frontmatter_value(text, "arxiv_id")),
...
def _semantic_scholar_enrich(candidates: list[dict[str, Any]], wiki_profile: dict[str, Any], cfg: dict[str, Any]) -> list[str]:
    ...
    if anchors:
        ...
        notes.append("Semantic Scholar recommendations skipped: no wiki ePrint anchors found.")
...
def _format_candidate(lines: list[str], index: int, paper: dict[str, Any]) -> None:
    ...
    lines.append(f"   - ePrint: [{arxiv_id}]({url})")
...
def _call_openai_compatible(
    *,
    ...
    system = (
        "You are OmegaWiki's daily ePrint recommendation judge. "
        "Use only the provided evidence. Return strict JSON only. "
        "This is inform mode: never select or request ingestion."
    )
    user = (
        "Rank fresh ePrint candidates for this user's research wiki. "
...
def _add_config_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--config", type=Path, default=Path("config/daily-eprint.yml"), help="Daily ePrint config path")
    ...
    parser.add_argument("--categories", nargs="*", help="Override ePrint categories")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="OmegaWiki daily ePrint helpers")
    sub = parser.add_subparsers(dest="command", required=True)

    config = sub.add_parser("config", help="Resolve daily ePrint config")
    ...
    prepare.add_argument("--feed", type=Path, help="JSON feed from tools/fetch_eprint.py")
    ...
    digest.add_argument("--feed", type=Path, required=True, help="JSON feed from tools/fetch_eprint.py")
...
def main() -> None:
    ...
    try:
        ...
        print(f"daily-eprint error: {exc}", file=sys.stderr)
        sys.exit(1)
