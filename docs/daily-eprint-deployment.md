# Deploying `/daily-eprint` on GitHub Actions
...
4. **Run `/daily-eprint setup` once** in your local checkout. The skill auto-patches `.github/workflows/daily-eprint.yml` to expose `SEMANTIC_SCHOLAR_API_KEY` and `DEEPXIV_TOKEN` to the Python prepare step (without these the secrets stay invisible to the runner and the daily run rate-limits out). Commit any resulting workflow change. If you can't run the slash skill, hand-add this under the `daily-eprint:` job's `env:` block:
   ```yaml
   SEMANTIC_SCHOLAR_API_KEY: ${{ secrets.SEMANTIC_SCHOLAR_API_KEY }}
   DEEPXIV_TOKEN:            ${{ secrets.DEEPXIV_TOKEN }}
   ```

5. **SMTP secrets**, if `email.enabled: true` in `config/daily-eprint.yml`:
   `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `SMTP_FROM`, `DAILY_EPRINT_EMAIL_TO`.

6. **Verify with one manual dispatch** before relying on the cron:
   ```bash
   gh workflow run daily-eprint.yml --ref main
   gh run watch
   ```
...
- *Either* a new commit on `main` titled `daily-eprint auto-ingest`, *or* the step summary line "no wiki/raw changes were staged" — depending on whether any candidate cleared the high-confidence gate that day. Both are valid.
